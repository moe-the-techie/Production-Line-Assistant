...
PRODUCT_CODES = {
    ...
    "Return Linear Slot Diffuser": "LRD"
}
N_CORNERS = 4
...
def add_product(code, discount_inputted, quantity=1, product_count=1, current_workbooks=None):
    ...
    try:
        ...
        if code == "LRD":
            ...
            return_linear_slot_diffuser(n_slots, lsd_width, lsd_length, quantity, product_count, current_workbooks)
...
def linear_slot_diffuser(n_slots, lsd_width, lsd_length, quantity, product_count, current_workbooks):
    ...
    # Cost calculations for profiles, accessories, materials, overhead, and labor
    ...
    report_sheet.cell(row=report_index, column=10).value = str(
        round(outer_frame_price * outer_frame_size / 1000, 2) * quantity) + "SAR"
    report_sheet.cell(row=report_index, column=11).value = str(round(inner_frame_size, 2) * quantity) + "mm"
    report_sheet.cell(row=report_index, column=12).value = str(
        round(inner_frame_price * inner_frame_size / 1000, 2) * quantity) + "SAR"
    report_sheet.cell(row=report_index, column=13).value = str(round(louver_size, 2) * quantity) + "mm"
    report_sheet.cell(row=report_index, column=14).value = str(
        round(louver_price * louver_size / 1000, 2) * quantity) + "SAR"
    report_sheet.cell(row=report_index, column=15).value = str(round(pipe_size, 2) * quantity) + "mm"
    report_sheet.cell(row=report_index, column=16).value = str(
        round(pipe_price * pipe_size / 1000, 2) * quantity) + "SAR"
    report_sheet.cell(row=report_index, column=17).value = str(round(space_bar_amount, 2) * quantity) + "mm"
    report_sheet.cell(row=report_index, column=18).value = str(n_space_bars * quantity) + "bars"
    report_sheet.cell(row=report_index, column=19).value = str(
        round(space_bar_price * space_bar_amount / 1000, 2) * quantity) + "SAR"
    report_sheet.cell(row=report_index, column=20).value = str(round(powder_weight, 2) * quantity) + "kg"
    report_sheet.cell(row=report_index, column=21).value = str(
        round(powder_weight * POWDER_PRICE_PER_KG / 1000, 2) * quantity) + "SAR"
    report_sheet.cell(row=report_index, column=22).value = str(n_hanging_clamps * quantity) + "clamps"
    report_sheet.cell(row=report_index, column=23).value = str(
        round(n_hanging_clamps * hanging_clamp_price, 2) * quantity) + "SAR"
    report_sheet.cell(row=report_index, column=24).value = aluminum_strap_type
    report_sheet.cell(row=report_index, column=25).value = str(n_aluminum_straps * quantity) + "straps"
    report_sheet.cell(row=report_index, column=26).value = str(
        round(n_aluminum_straps * aluminum_strap_price, 2) * quantity) + "SAR"
...
def return_linear_slot_diffuser(n_slots, lrd_width, lrd_length, quantity, product_count, current_workbooks):
    """
    Calculates the costs of production for the Linear Slot Diffuser and exports it into an Excel file.
    :param n_slots: integer representing the number of slots
    :param lrd_width: float representing the size of the gap in millimeters
    :param lrd_length: float representing the length of the linear slot diffuser in millimeters
    :param quantity: integer representing the quantity of LSDs required
    :param current_workbooks: a tuple containing two openpyxl workbooks each representing a report file to be saved
    :param product_count: an integer representing the number of products entered so far
    :return:
    ...
    # Calculations for: outer & inner frames, space bar, powder time, hanging clamps, and pipe

    n_inner_frames = n_slots - 1
    inner_frame_thickness = 1.2
    outer_frame_neck_thickness = 4.4

    end_cap_size = (lrd_width + 16) * n_slots + n_inner_frames * inner_frame_thickness + 2 * outer_frame_neck_thickness

    n_pipes = int((lrd_length - 200) / 350) + 1
    pipe_size = n_pipes * end_cap_size

    space_bar_amount = lrd_length / 350 * end_cap_size - 5
    n_space_bars = pipe_size - 5
    outer_frame_size = lrd_length * 2 + end_cap_size * 2 + 380
    inner_frame_size = n_inner_frames * lrd_length

    n_hanging_clamps = int(lrd_length / 400)
    ...
    louver_size = n_slots * 2 * lrd_length

    powder_weight = 0.0001333333333 * lrd_length

    powder_time = ((90 / (6000 * 12 / lrd_length)) + (90 / (6000 * 36 / lrd_length)) + 1) * 2
    ...
    accessories_cost = n_hanging_clamps * hanging_clamp_price + powder_weight * POWDER_PRICE_PER_KG

    material_cost = profiles_cost + accessories_cost

    labor_cost = material_cost * 0.35 + powder_time
    ...
    # Entering the data into the right row, notice that the 1's represent the quantity and should be updated
    # to whatever value that ends up taking on
    invoice_sheet.cell(row=invoice_index, column=2).value = PRODUCT_CODES["Return Linear Slot Diffuser"]
    invoice_sheet.cell(row=invoice_index, column=3).value = "Return Linear Slot Diffuser"
    invoice_sheet.cell(row=invoice_index, column=7).value = lrd_length
    invoice_sheet.cell(row=invoice_index, column=8).value = quantity
    invoice_sheet.cell(row=invoice_index, column=12).value = unit_price
    invoice_sheet.cell(row=invoice_index, column=13).value = unit_price - (unit_price * discount / 100)
    invoice_sheet.cell(row=invoice_index, column=14).value = unit_price * quantity

    report_sheet.cell(row=report_index, column=1).value = PRODUCT_CODES["Return Linear Slot Diffuser"]
    report_sheet.cell(row=report_index, column=2).value = quantity
    report_sheet.cell(row=report_index, column=3).value = str(round(total_cost, 2) * quantity) + "SAR"
    report_sheet.cell(row=report_index, column=4).value = str(round(profiles_cost, 2) * quantity) + "SAR"
    report_sheet.cell(row=report_index, column=5).value = str(round(accessories_cost, 2) * quantity) + "SAR"
    ...
    report_sheet.cell(row=report_index, column=20).value = str(round(powder_weight, 2) * quantity) + "kg"
    report_sheet.cell(row=report_index, column=21).value = str(
        powder_weight * POWDER_PRICE_PER_KG / 1000 * quantity) + "SAR"
    report_sheet.cell(row=report_index, column=22).value = str(n_hanging_clamps * quantity) + "clamps"
    report_sheet.cell(row=report_index, column=23).value = str(
        round(n_hanging_clamps * hanging_clamp_price, 2) * quantity) + "SAR"
    report_sheet.cell(row=report_index, column=24).value = "N/A"
    report_sheet.cell(row=report_index, column=25).value = "N/A"
    report_sheet.cell(row=report_index, column=26).value = "N/A"
